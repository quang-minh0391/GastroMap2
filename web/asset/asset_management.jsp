...
<style>
    ...
    /* Form Styles */
    .form-asset input, .form-asset select {
        width: 100%;
        padding: 8px;
        margin: 5px 0 15px;
...
<div class="container-fluid p-4">
    ...
    <div class="main-content-asset">
        ...
        <div class="table-section-asset">
            ...
            <form id="filterForm" action="AssetServlet" method="GET" class="filter-wrapper">
                <input type="hidden" id="sortBy" name="sortBy" value="${sortBy}">
                <input type="hidden" id="sortOrder" name="sortOrder" value="${sortOrder}">

                <input type="text" class="filter-input" name="f_code" value="${f_code}" placeholder="Mã TS..." style="width: 100px;">

                <input type="text" class="filter-input" name="f_name" value="${f_name}" placeholder="Tên thiết bị..." style="min-width: 180px;">

                <select class="filter-input" name="f_status">
                    <option value="">-- Trạng thái --</option>
                    <option value="ACTIVE" ${f_status == 'ACTIVE' ? 'selected' : ''}>Đang sử dụng</option>
                    <option value="MAINTENANCE" ${f_status == 'MAINTENANCE' ? 'selected' : ''}>Bảo trì</option>
                    <option value="BROKEN" ${f_status == 'BROKEN' ? 'selected' : ''}>Hỏng</option>
                ...
                <div class="filter-group">
                    <span class="group-label">📅 Ngày mua</span>
                ...
                <div class="filter-group">
                    <span class="group-label">💰 Giá</span>
                    <input type="number" name="f_price_from" value="${f_price_from}" placeholder="Từ..." min="0" step="1000" style="width: 90px;">
                    <span class="group-divider">-</span>
                    <input type="number" name="f_price_to" value="${f_price_to}" placeholder="Đến..." min="0" step="1000" style="width: 90px;">
                </div>

                <button type="submit" class="filter-btn">🔍 Lọc</button>
                <a href="AssetServlet" class="filter-clear">✖ Xóa lọc</a>
            </form>
            ...
            <table class="table-asset">
                            ...
                            <td>
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <button type="button" onclick="openLiquidateModal('${a.id}', '${a.name}')"
                                                style="background: #6c757d; color: white; border:none; padding:4px 8px; border-radius: 4px; font-size: 11px; cursor:pointer; width: auto; margin-left: 5px;">
                                            💰 Thanh lý
                                        </button>
                                    </c:otherwise>
                                </c:choose>
                ...
                <div id="liquidateModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5);">
                    <div style="background:white; margin:10% auto; padding:20px; width:400px; border-radius:8px; box-shadow:0 5px 15px rgba(0,0,0,0.3);">
                        <h3 style="margin-top:0;">💰 Thanh lý Tài sản</h3>
                        <form action="AssetServlet" method="POST">
                            <input type="hidden" name="action" value="liquidate">
                            <input type="hidden" name="asset_id" id="liq_asset_id">
                            <input type="hidden" name="asset_name" id="liq_asset_name">

                            <p>Bạn đang thanh lý: <b id="liq_display_name" style="color:#dc3545"></b></p>

                            <label style="display:block; margin-top:10px;">Giá bán thanh lý (VNĐ):</label>
                            <input type="number" name="price" id="liq_price" required min="0" value="0" style="width:100%; padding:8px; margin-bottom:15px; border: 1px solid #ccc; border-radius: 4px;">

                            <div style="text-align:right;">
                                <button type="button" onclick="document.getElementById('liquidateModal').style.display = 'none'" style="background:#ccc; color: #333; border:none; padding:8px 15px; cursor:pointer; border-radius:4px; margin-right: 5px;">Hủy</button>
                                <button type="submit" style="background:#6c757d; color:white; border:none; padding:8px 15px; cursor:pointer; border-radius:4px;">Xác nhận Thanh lý</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <script>
                    // Hàm mở modal mua lại (Giữ nguyên của bạn)
                    function openRepurchaseModal(id, name, currentValue) {
                        ...
                        document.getElementById('modal_price').value = currentValue;
                        document.getElementById('repurchaseModal').style.display = 'block';
                    }

                    // [MỚI] Hàm mở modal thanh lý
                    function openLiquidateModal(id, name) {
                        document.getElementById('liq_asset_id').value = id;
                        document.getElementById('liq_asset_name').value = name;
                        document.getElementById('liq_display_name').innerText = name;
                        document.getElementById('liq_price').value = 0; // Mặc định giá thanh lý gợi ý là 0
                        document.getElementById('liquidateModal').style.display = 'block';
                    }
                </script>
                </tbody>
            </table>
...
<c:if test="${not empty sessionScope.message}">
    <div id="successModal" class="modal" style="display:block; position:fixed; z-index:9999; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5);">
        <div style="background:white; margin:15% auto; padding:30px; width:350px; border-radius:12px; text-align:center; box-shadow:0 10px 25px rgba(0,0,0,0.2); animation: popIn 0.3s ease-out;">
            
            <div style="color:#28a745; font-size: 60px; line-height: 1; margin-bottom: 10px;">✔</div>
            <h3 style="margin-top:0; color: #333; font-size: 22px;">Thành công!</h3>
            
            <p style="color: #555; font-size: 16px; margin-bottom: 25px;">${sessionScope.message}</p>
            
            <button onclick="document.getElementById('successModal').style.display='none'" 
                    style="background:#28a745; color:white; border:none; padding:10px 30px; border-radius:6px; font-size:16px; cursor:pointer; font-weight: bold;">
                Đóng
            </button>
        </div>
    </div>
    ...
    <%-- Xóa biến message khỏi session để F5 không bị hiện lại --%>
